...
public class IncreaseMoneyRequestService implements IncreaseMoneyRequestUseCase {
    ...
    public MoneyChangingRequest increaseMoneyRequest(IncreaseMoneyRequestCommand command) {
        ...
        // 2. 고객의 연동된 계좌가 있는지, 그리고 그 계좌의 잔액이 충분한지도 확인(뱅킹)

        // 3. 법인 계좌 상태도 정상인지 확인(뱅킹)

        // 4. 증액을 위한 "기록", 요청 상태로 MoneyChangingRequest 를 생성한다.(MoneyChangingRequest)

        // 5. 펌뱅킹을 수행하고 (고객의 연동된 계좌 -> 내 페이의 법인 계좌)(뱅킹)
        ...
        if(memberMoneyJpaEntity != null) {
            MoneyChangingRequest moneyChangingRequest = MoneyChangingRequest.builder()
                    ...
                    .build();
            MoneyChangingRequestJpaEntity requestJpaEntity = increaseMoneyPort.createMoneyChangingRequest(
                    moneyChangingRequest
            );
            return moneyChangingRequestMapper.mapToDomain(requestJpaEntity);
        }

        // 6-2. 결과가 실패라면, null 리턴(아직 펌뱅킹의 실패 처리 로직이 없으므로)
        return null;
    }
}
